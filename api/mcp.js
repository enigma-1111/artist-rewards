...
function bankr(handle, amount) {
  ...
  const amt = String(amount || "500 ART");
...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        face: { type: "string", enum: ["all", "pbs", "cdn", "initials"], default: "all" },
      },
    },
  },
  {
...
function callTool(name, args) {
  ...
  if (name === "get_art_status") {
    return {
      ...STATUS,
      artists: rows().length,
      initials: rows().filter((r) => r.face === "initials").length,
      lookalikeWarning:
        "ScanHood symbol ART is a different token named Robinhood ART. Do not paste that 0x into CONFIG.contract.",
    };
  }
  if (name === "list_artists") {
    const want = args.face || "all";
    const list = rows().filter((r) => want === "all" || r.face === want);
    return { count: list.length, artists: list };
  }
  ...
  if (name === "list_collections") {
    return { count: COLLECTIONS.length, collections: COLLECTIONS, affiliation: "none" };
  }
  ...
  if (name === "parse_nft_tip") {
    ...
    const item = raw.match(/opensea\.io\/(?:assets|item)\/([^/]+)\/(0x[a-fA-F0-9]{40})\/(\d+)/i);
    if (item) {
      return { kind: "item", chain: item[1], contract: item[2], tokenId: item[3] };
    }
    const pair = raw.match(/(0x[a-fA-F0-9]{40})\s*[/, ]\s*(\d+)/);
    if (pair) {
      return { kind: "pair", contract: pair[1], tokenId: pair[2] };
    }
    const col = raw.match(/opensea\.io\/collection\/([a-z0-9-]+)/i);
    if (col) {
